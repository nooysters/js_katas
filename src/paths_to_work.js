...
function pathsToWork(x, y) {
  ...
  let map = Array(x+1).fill().map( ()=>{ return Array(y+1).fill(0) } )
  let routes = 0
  map[x][y] = 1 // sets work

  searchRoute(0, 0)

  function searchRoute(s, e) {
    if( !map[s] ) return
    if( map[s][e] === 1 || s === x || e === y ) return routes ++
    if( map[s][e] === 0 ) searchRoute(s+1, e), searchRoute(s, e+1)
  }
